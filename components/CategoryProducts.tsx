      ...
      <div className="flex flex-col md:min-w-40 border">
        {categories?.filter((item) => item?.slug?.current !== "mobiles").reduce((acc: Category[], item, index) => {
          ...
          <Button
            ...
            key={item?._id}
            className={`bg-transparent border-0 p-0  rounded-none text-darkColor dark:text-white shadow-none hover:bg-shop_orange hover:text-white font-semibold hoverEffect border-b last:border-b-0 transition-colors capitalize ${item?.slug?.current === currentSlug && "bg-shop_orange text-white border-shop_orange"}`}
          >
